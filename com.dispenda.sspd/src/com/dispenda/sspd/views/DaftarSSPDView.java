...
public class DaftarSSPDView extends ViewPart {
	public DaftarSSPDView() {
	}
	
	public static final String ID = DaftarSSPDView.class.getName();
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (cmbKewajibanPajak.getSelectionIndex() >= 0){
					if(cmbTipeSKP.getSelectionIndex()>=0)
						result = ControllerFactory.getMainController().getCpSspdDAOImpl().getDaftarSSPD(cmbTipeSKP.getItem(cmbTipeSKP.getSelectionIndex()),cmbKewajibanPajak.getData(Integer.toString(cmbKewajibanPajak.getSelectionIndex())).toString(), masaPajakDari, masaPajakSampai);
					else
						result = ControllerFactory.getMainController().getCpSspdDAOImpl().getDaftarSSPD("",cmbKewajibanPajak.getData(Integer.toString(cmbKewajibanPajak.getSelectionIndex())).toString(), masaPajakDari, masaPajakSampai);
				}else
					result = ControllerFactory.getMainController().getCpSspdDAOImpl().getDaftarSSPDAll(masaPajakDari, masaPajakSampai, cmbTipeSKP.getItem(cmbTipeSKP.getSelectionIndex()));
		...
		btnCetak.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if(isPrint()){
					...
					ReportAppContext.classLoader = DaftarSSPDView.class.getClassLoader();
					...
					for(int ti=0;ti<tbl_SSPD.getItems().length - 1; ti++){
						...
						for(int i=0;i<tbl_SSPD.getColumnCount();i++){
							if (i == 10 || i == 11 || i == 12 || i == 13 || i == 14)
								if (tbl_SSPD.getItem(ti).getText(i).startsWith("-"))
									values.add(tbl_SSPD.getItem(ti).getText(i).substring(3).replace(".", "").replace(",", "."));
								else
					...
					ReportAction.start("DaftarSSPD.rptdesign");
				}else
	...
	private void createColumn()
	{
		if(tbl_SSPD.getColumnCount() <= 0){
			String[] listColumn = {"Tgl SPTPD","No SSPD", "No Reg", "Tanggal SSPD", "Tanggal Cetak", "NPWPD", "Nama Badan", "Nama Pajak", "Bidang Usaha", "No SKP", "Pokok Pajak", 
					"Denda", "Denda Terlambat Bayar", "Denda Dibayar", "Jumlah Bayar", "Bulan SKP", "Nama Penyetor"};
			int[] widthColumn = {130, 130, 100, 115, 120, 120, 160, 140, 160, 150, 150, 140, 150, 150, 150, 90, 150};
...
//					item.setText(0, result.getString("TGL_SPTPD"));
				...
				item.setText(4, formatter.format(result.getDate("Tgl_Cetak")));
				item.setText(5, result.getString("NPWPD"));
				item.setText(6, result.getString("Nama_Badan"));
				item.setText(7, result.getString("Nama_Pajak"));
				item.setText(8, result.getString("Bidang_Usaha"));
				item.setText(9, result.getString("No_SKP"));
				item.setText(10, indFormat.format(roundUP(result.getDouble("PokokPajak"))));
				item.setText(11, indFormat.format(roundUP(result.getDouble("Denda"))));
				item.setText(12, indFormat.format(roundUP(result.getDouble("DendaTerlambatbayar"))));
				item.setText(13, indFormat.format(roundUP(result.getDouble("Jumlah_Bayar")) - roundUP(result.getDouble("PokokPajak"))));
				item.setText(14, indFormat.format(roundUP(result.getDouble("Jumlah_Bayar"))));
				item.setText(15, result.getString("Bulan_SKP"));
				item.setText(16, result.getString("Nama_Penyetor"));
				try {
					totalPokokPajak += indFormat.parse(item.getText(10)).doubleValue();
					totalDenda += indFormat.parse(item.getText(11)).doubleValue();
					totalDendaTerlambat += indFormat.parse(item.getText(12)).doubleValue();
					totalDendaDibayar += indFormat.parse(item.getText(13)).doubleValue();
					totalJumlahBayar += indFormat.parse(item.getText(14)).doubleValue();
			...
			item.setText(10, indFormat.format(totalPokokPajak));
			item.setText(11, indFormat.format(totalDenda));
			item.setText(12, indFormat.format(totalDendaTerlambat));
			item.setText(13, indFormat.format(totalDendaDibayar));
